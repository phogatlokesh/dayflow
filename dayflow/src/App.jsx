...
function App() {
  ...
  const submitForm = async (event) => {
    ...
    try {
      ...
      if (view === 'signin') { setIsLoggedIn(true); setUserRole(data.user?.role || 'Employee'); setUserDesignation(data.profile?.role || 'Software Engineer'); setUserId(data.user?.employee_id || form.employeeId); setAuthToken(data.token || ''); setUserProfile(data.profile || null); return }
