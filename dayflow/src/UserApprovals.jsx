...
function UserApprovals({ token }) {
  ...
  const load = async () => { try { const response = await fetch('/api/admin/pending-users', { headers: { Authorization: `Bearer ${token}` } }); const data = await response.json(); if (!response.ok) throw new Error(data.message); setUsers(data.users); } catch (error) { setMessage(error.message); } finally { setLoading(false) } }
  useEffect(() => { load() }, [])
  const decide = async (employeeId, status) => { const response = await fetch(`/api/admin/pending-users/${employeeId}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ status }) }); const data = await response.json(); if (!response.ok) { setMessage(data.message); return } setUsers((current) => current.filter((user) => user.employee_id !== employeeId)); setMessage(`Employee ${status}. They have been notified through their account status.`) }
